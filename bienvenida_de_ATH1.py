    ...
    if os.path.exists("ath1_voice_templates.json"):
        with open("ath1_voice_templates.json", "r") as f:
            try:
                ...
                print(f"⚠️ Error al cargar huellas de voz: {e}")
    else:
        print("⚠️ ALERTA: No tienes huellas grabadas. Ejecuta primero 'entrenar_nombre.py'")

    # Importamos las funciones optimizadas desde tu módulo de entrenamiento
    from entrenar_nombre import extraer_espectrograma, calcular_distancia_dtw

    # ─── VARIABLES DE CONTROL DE ATENCIÓN CONTINUA ───
    asistente_activo = False
    tiempo_ultima_orden = 0
    TIEMPO_ESPERA_MAXIMO = 40.0  # 40 segundos en modo activo
